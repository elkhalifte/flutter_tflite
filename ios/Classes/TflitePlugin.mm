...
bool isFloat = false;
std::vector<std::vector<float>> labelProb;
...
NSString* loadModel(NSObject<FlutterPluginRegistrar>* _registrar, NSDictionary* args) {
  ...
  NSNumber* isFloatNumber = args[@"isFloat"];
  isFloat = [isFloatNumber boolValue];
  if(isAsset){
  ...
  if ([args[@"passedLabels"] length] > 0) {
    // NSString* labels_path;
    // if(isAsset){
    //   key = [_registrar lookupKeyForAsset:args[@"passedLabels"]];
    //   labels_path = [[NSBundle mainBundle] pathForResource:key ofType:nil];
    // }else{
    //   labels_path = args[@"passedLabels"];
    // }
    labels.clear();
    NSString *labelsString = args[@"passedLabels"]; // Direct access
    if (![labelsString isKindOfClass:[NSString class]]) {
    labelsString = @"";  // Default empty string if invalid
}
    // LoadLabels(&labels);
        std::string labelsStr = [labelsString UTF8String];
        std::stringstream ss(labelsStr);
        std::string label;
        ...
        if (isFloat) {
    labelProb.clear();
    labelProb.push_back(std::vector<float>(labels.size(), 0.0f)); // 1 row, N columns
} else {
    labelProbByte.clear();
    labelProbByte.push_back(std::vector<uint8_t>(labels.size(), 0)); // 1 row, N columns
}
